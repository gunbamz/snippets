...
const BinanceFOrder = async (a) => {
  let b = a.split(" ");
  const  timestamp = Date.now();
  let param = `symbol=${b[4]}&side=${b[1]}&positionSide=${b[2]}&type=${b[3]}&quantity=${b[6]}&price=${b[5]}&stopPrice=${b[0]}&timeInForce=${b[7]}&recvWindow=20000&timestamp=${timestamp}`;
  ...
  params.append('quantity', b[6]);
  params.append('price', b[5]);
  params.append('stopPrice', b[0]);
  params.append('timeInForce', b[7]);
